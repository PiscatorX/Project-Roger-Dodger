...
(RF_var_importance <- read.table("RF_feature_importance_PI.tsv", header = T, stringsAsFactors = F))

top_vars <- RF_var_importance  %>% 
                 filter(importance_mean >= 0.01)
...
get_factors <- function(df, col){

  data.frame(unique(df[col]))
...
Adsorbent <-  get_factors(zeo_cat, "Adsorbent")
Adsorbate <-  get_factors(zeo_cat, "S")
solvent <-  get_factors(zeo_cat, "solvent")

(solvent_varimp <- RF_var_importance[RF_var_importance$features %in% solvent$solvent,])
(Adsorbate_varimp <-  RF_var_importance[RF_var_importance$features %in% Adsorbate$S,])
(Adsorbent_varimp <- RF_var_importance[RF_var_importance$features %in% Adsorbent$Adsorbent,])
...
colSums(zeolite_properties_varimp[c(2:3)])
colSums(zeolite_metals_varimp[c(2:3)])
colSums(zeolite_conditions_varimp[c(2:3)])

colSums(zeolite_properties_varimp_ALL[c(2:3)])
colSums(zeolite_conditions_varimp_ALL[c(2:3)])
...
zeolite_main <- zeolite_main %>%
                arrange(desc(importance_mean)) %>%
                mutate(perc = round(importance_mean/sum(importance_mean) * 100, 1)) %>%
...
ggplot(zeolite_main_prop, aes(x = 2,  y = perc,  fill = features)) +
        geom_bar(width = 1,  stat = "identity") +
        labs(x = NULL, y = NULL, fill = "Zeolite properities") +
        guides(fill = guide_legend(reverse = TRUE)) +
        geom_text_repel(aes(y = lab_pos,
        ...
        scale_fill_lancet(labels = c("Adsorbent", "Characteristics", "Metal ion")) +
        coord_polar(theta = "y") +
        xlim(0.5,2.5)
...
zeolite_main_conditions <- rbind(Conditions_main, solvent_main) %>% rownames_to_column("features")
